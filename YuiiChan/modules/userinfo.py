...
import YuiiChan.modules.sql.userinfo_sql as sql
...
from YuiiChan.modules.sql.afk_sql import is_afk, check_afk_status
from YuiiChan.modules.sql.users_sql import get_user_num_chats
from YuiiChan.modules.sql.feds_sql import get_user_fbanlist
...
from YuiiChan import telethn as SaitamaTelethonClient, TIGER_USERS, SUDO_USERS, SUPPORT_USERS
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nThis user is member of 'Yui-Chan Club'."
        disaster_level_present = True
    elif user.id in SUDO_USERS:
        text += "\n\nThis person is one of my 'sudo'."
        disaster_level_present = True
    elif user.id in SUPPORT_USERS:
        text += "\n\nThis person is with the 'Support'."
        disaster_level_present = True
    elif user.id in TIGER_USERS:
        text += "\n\nThis person is one of my'sudo'."
        disaster_level_present = True
    elif user.id in WHITELIST_USERS:
        text += "\n\nThhis person is one of 'Whitelist User."
        disaster_level_present = True
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in SUDO_USERS and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust YuiChanClub members to set my bio.")
            return
