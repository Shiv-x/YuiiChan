...
from YuiiChan.modules.helper_funcs.extraction import extract_user_and_text
from YuiiChan.modules.helper_funcs.string_handling import extract_time
from YuiiChan.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                "Trying to put me against my master and my owner huh?")
            return log_message
            ...
            message.reply_text(
                "Why you a fighting with sudos they have guns you know"
            )
            return log_message
        elif user_id in SUPPORT_USERS:
            message.reply_text(
                "Can't bang Support users reeee."
            )
            return log_message
        elif user_id in TIGER_USERS:
            message.reply_text(
                "That's a demon!\nYou need to ask at @yuiichansupport to handle those."
            )
            return log_message
        elif user_id in WHITELIST_USERS:
            message.reply_text(
                "That's a whitelist user!\nYou need to put a orders through YuiChanClub to handle those."
            )
            return log_message
        else:
